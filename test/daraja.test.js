...
let insufficientBalanceRequestBody = {
  Body: {
    stkCallback: {
      ...
      ResultCode: 1,
      ResultDesc:
        '[MpesaCB - ]The balance is insufficient for the transaction.',
    },
  },
};
  ...
  it('it should return (500) on invalid parameters -> invalid Phonenumber', done => {
    request(server)
      ...
      .end((err, res) => {
        ...
        assert(res.body.errorMessage).to.eql(
          'Bad Request - Invalid PhoneNumer',
        );
  ...
  it('it should return (200) on successful transaction processing', done => {
    request(server)
      ...
      .end((err, res) => {
        ...
        assert(res.body.transactionStatus).to.eql(
