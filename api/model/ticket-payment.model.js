...
const paymentsCollection = database.collection(
  process.env.NODE_ENV === 'test' ? 'events_payments_test' : 'events_payments',
);
const moment = require('moment');
const Utils = require('./utils/utils');
const failedTransactions = database.collection(
  process.env.NODE_ENV === 'test'
    ? 'failed_events_payments_test'
    : 'failed_event_payments',
);

exports.savePaymentTransaction = async (
...
exports.recordFailedTransaction = async (
  studentAdmissionNumber,
  eventId,
  failureQuery,
) => {
...
exports.fetchTransactionByAdmissionNumber = async admissionNumber => {
  return await paymentsCollection
    .where('studentAdmissionNumber', '==', studentAdmissionNumber)
    .get();
};

exports.fetchFailedTransactionsByAdmisionNumber = async admissionNumber => {
  return await failedTransactions
    .where('studentAdmissionNumber', '==', admissionNumber)
    .get();
};
...
exports.clearPaymentsCollection = () => {
  ...
  let query = paymentsCollection
    .orderBy('studentAdmissionNumber')
    .limit(batchSize);
