...
module.exports = class ExcelParser {
  ...
  sanitize(text) {
    ...
    if (courseCode.includes('/')) {
      let courseCodes = [];
      if (courseCode.match(similarDoubleClasses)) {
        // handle type YYY111A/YYY222A/YYY333
        courseCodes = courseCode
          .split('/')
          .map(code => (isNaN(code.substr(-1)) ? code : this.addSection(code)));
      } else if (courseCode.match(lackingClassPrefix)) {
        // handle type YYY111/YYY222A
        courseCodes = courseCode.split('/');
        courseCodes[0] = courseCodes[0].concat(courseCode.substr(-1));
      } else if (courseCode.match(conjoinedClasses)) {
        // handle type YYY111A/B
        ...
        codes.forEach(code => {
          ...
          if (isNaN(last)) {
            ...
            section =
              this.shift === 'athi' ? 'A' : this.shift === 'day' ? 'T' : 'X';
          }
